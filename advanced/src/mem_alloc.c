...
void run_at_exit(void)
{
    ...
    if(flag)
    {
      printf("WARNING - Block address : %lu was not freed \n", ULONG((void *)tmp + sizeof(mem_used_block_t) - heap_start));
    }
    else
    {
...
void *memory_alloc(size_t size)
{
    ...
    // Alignment
    if((size + sizeof(mem_used_block_t)) % MEM_ALIGNMENT != 0)
    {
      padding = MEM_ALIGNMENT - ((size + sizeof(mem_used_block_t)) % MEM_ALIGNMENT);
      size += padding;
    }
    ...
    if(free->size >= (size + sizeof(mem_used_block_t)))
    {
      ...
      print_alloc_info((void*)tmp + sizeof(mem_used_block_t), size - padding);
      update_free_list();
    }
    else
    {
      print_alloc_error(size - padding);
    }
